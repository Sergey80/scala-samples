...
object ThreadStarvation extends App {
    /*
        ...
        where n is amount of cores on particular machine
      */
...
//  val time1 = measure {
    ...
    val works1 = (1 to cores * 2) map { x =>
      Future {
        Thread.sleep(1000) // each thread takes 1 second to run
        println(s"work $x is done")
      }
    }
    Await.ready(Future.sequence(works1), Duration.Inf) // half of all threads will be execution in parallel (another half will be waiting for its turn)
...
//  val time2 = measure {
    ...
    Await.ready(Future.sequence(works2), Duration.Inf)
//  }
...
//val time3 = measure {
  val works: IndexedSeq[Future[Unit]] = (1 to cores * 10000).map { x =>   // 10000 more than cores!
    Future {
      blocking {                   // !
        Thread.sleep(1000)
        println(s"work $x is done")
